...
#         raise RuntimeError("Sampling rate of the file {} is {} Hz, but the model requires {} Hz".format(full_path, sampling_rate, sr_target))
...
class UnitDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        audio, _ = load_wav(data_item["path"], self.sampling_rate) # [1, Ta]
        ...
        assert (
            acoustic_tokens.shape[0] >= self.codebook_num
        ), f"codebook_num {self.codebook_num} exceeds acoustic token codebook num {acoustic_tokens.shape[0]}"
        ...
        code_length = min(
            audio.shape[-1] // self.unit_hop_size, acoustic_tokens.shape[-1]
        )
        ...
        if self.split:
            ...
            assert (
                self.segment_size == unit_segment_size * self.unit_hop_size
            ), f"segment_size {self.segment_size} cannot be devided by unit_hop_size{self.unit_hop_size}"
            ...
            acoustic_tokens = acoustic_tokens[
                :, unit_start : unit_start + unit_segment_size
            ]
            audio = audio[:, audio_start : audio_start + self.segment_size]
        else:
            assert (
                audio.shape[-1] == acoustic_tokens.shape[-1] * self.unit_hop_size
            ), "audio shape {} acoustic shape {}".format(
                audio.shape, acoustic_tokens.shape
            )
